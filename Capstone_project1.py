...
# Daftar data dari toko rental Mobil
data_rental = [
    {
        "id_mobil": 101,
        "tipe_mobil": "Mustang",
    ...
    while True:
        ...
        if pilih_submenu == "1":
            ...
            if not validasi_id(id_mobil):
                print("ID Mobil harus 3 angka, kembali ke submenu.")
                continue  

            if any(rental['id_mobil'] == int(id_mobil) for rental in data_rental):
                print("Mobil dengan ID ini sudah disewa, kembali ke submenu.")
                continue  

            tipe_mobil = input("Masukkan Tipe Mobil (hanya huruf): ")
            if not validasi_nama(tipe_mobil):
                print("Tipe Mobil hanya boleh berisi huruf, kembali ke submenu.")
                continue  

            harga_sewa = input("Masukkan Harga Sewa per Hari (hanya angka): ")
            if not validasi_harga(harga_sewa):
                print("Harga sewa harus berupa angka, kembali ke submenu.")
                continue  

            id_pelanggan = input("Masukkan Nomor ID Pelanggan (5 digit): ")
            if not validasi_idpelanggan(id_pelanggan):
                print("Nomor ID Pelanggan harus 5 digit angka, kembali ke submenu.")
                continue  

            nama_pelanggan = input("Masukkan Nama Pelanggan (Nama Depan dan Belakang): ")
            if not validasi_nama(nama_pelanggan):
                print("Nama Pelanggan hanya boleh berisi huruf, kembali ke submenu.")
                continue 
        ...
        if pilihan_submenu == "1":
            ...
            for rental in data_rental:
                if rental["id_mobil"] == int(id_mobil):
                    ...
                    print("\nMasukkan data baru (kosongkan untuk tidak mengubah data!):")
                    ...
                    if id_baru:
                        if validasi_idpelanggan(id_baru):
                            ...
                            print("Nomor ID pelanggan tidak valid!")
                    ...
                    if konfirmasi == 'Y':
                        ...
                        print("Perubahan data dibatalkan, kembali ke submenu.")
                        break
                    else:
...
def main():
    while True:
        ...
        if choice == "1":
            ...
            print("Terima Kasih, Silahkan Kembali!!.")
            break
        else:
